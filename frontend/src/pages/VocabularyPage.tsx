...
export default function VocabularyPage() {
  ...
  return <div className="page vocabulary-page">
    <header className="page-title"><span className="page-title-icon"><BookmarkSimpleIcon weight="fill"/></span><h1>내 단어장</h1><strong>{saved.length}개</strong></header>
    <header className="vocabulary-toolbar"><div className="vocabulary-heading"><div className={`vocabulary-view-menu${viewMenuOpen ? ' open' : ''}`} ref={viewMenuRef}><button type="button" className="vocabulary-view-trigger" aria-haspopup="listbox" aria-expanded={viewMenuOpen} onClick={() => setViewMenuOpen(open => !open)}><span>{views.find(item => item.id === view)?.label}</span><CaretDownIcon weight="bold"/></button>{viewMenuOpen && <div className="vocabulary-view-options" role="listbox" aria-label="단어 표시 방식">{views.map(item => <button type="button" role="option" aria-selected={view === item.id} className={view === item.id ? 'active' : ''} onClick={() => { setView(item.id); setViewMenuOpen(false) }} key={item.id}><span>{item.label}</span></button>)}</div>}</div></div><label className="vocabulary-search"><MagnifyingGlassIcon/><input value={search} onChange={event => setSearch(event.target.value)} placeholder="단어나 뜻을 검색하세요" aria-label="단어장 검색"/></label><div className="vocab-filters" role="tablist" aria-label="단어장 필터">{filters.map(item => <button role="tab" aria-selected={filter === item.id} className={filter === item.id ? 'active' : ''} onClick={() => setFilter(item.id)} key={item.id}>{item.id === 'all' ? <ListBulletsIcon weight="bold"/> : item.id === 'favorite' ? <HeartIcon weight="fill"/> : item.id === 'due' ? <ClockCounterClockwiseIcon weight="bold"/> : <TrophyIcon weight="fill"/>}{item.label}</button>)}</div></header>
          ...
          : !visibleWords.length ? <div className="empty-state vocabulary-empty"><MagnifyingGlassIcon/><h2>조건에 맞는 단어가 없어요</h2><p>검색어나 필터를 바꿔보세요.</p></div>
            : <div className="word-grid compact-word-grid">{visibleWords.map(word => {
              ...
              return <article className={`word-card compact-word-card${expanded ? ' expanded' : ''}`} key={word.vocabularyId}>
                ...
                {expanded && <div className="word-details">{word.contextMeaning && <div><b>문맥 뜻</b><p>{word.contextMeaning}</p></div>}{word.exampleSentence && <div><div className="word-detail-heading"><b>예문</b><div><button type="button" aria-label={`${word.word} 예문 듣기`} onClick={() => speak(word.exampleSentence!, word.languageCode)}><SpeakerHighIcon/></button>{word.exampleTranslation && <button type="button" className={translatedExamples.has(word.vocabularyId) ? 'active' : ''} aria-label={`${word.word} 예문 해석 ${translatedExamples.has(word.vocabularyId) ? '숨기기' : '보기'}`} aria-pressed={translatedExamples.has(word.vocabularyId)} onClick={() => toggleExampleTranslation(word.vocabularyId)}><TranslateIcon/></button>}</div></div><blockquote>{word.exampleSentence}</blockquote>{translatedExamples.has(word.vocabularyId) && word.exampleTranslation && <p className="word-example-translation">{word.exampleTranslation}</p>}</div>}{word.etymology && <div><b>어원</b><p>{word.etymology}</p></div>}{word.memoryTip && <div className="word-memory-tip"><b>암기 팁</b><p>💡 {word.memoryTip}</p></div>}</div>}
              </article>
            })}</div>}
