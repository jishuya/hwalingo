...
export default function StudyPage({ analyze }: { analyze: () => void }) {
  ...
  return <div className="page narrow"><section className="card input-card"><div className="study-heading"><h1>학습할 언어와 문구를 입력하세요</h1><div className="language-selectors"><LanguageSelect defaultValue="ko" ariaLabel="변경 전 언어"/><span className="language-arrow" aria-hidden="true"><ArrowRightIcon weight="bold" /></span><LanguageSelect defaultValue="en" ariaLabel="변경 후 언어"/></div></div><textarea value={text} onChange={event => setText(event.target.value)} placeholder="학습하고 싶은 문장이나 문구를 입력해 주세요."/><div className="input-footer"><span>{text.length} / 500</span><button className="primary" onClick={analyze}><Icon>✦</Icon> AI 분석 시작</button></div></section><div className="empty-state"><span>✎</span><p>AI가 표현과 핵심 어휘를 분석해 드려요.</p></div></div>
}
