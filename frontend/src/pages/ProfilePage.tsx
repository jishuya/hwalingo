import { BookOpenIcon, ChartBarIcon, ClockCounterClockwiseIcon, FireIcon, PencilSimpleIcon, StepsIcon, TrophyIcon } from '@phosphor-icons/react'
...
export default function ProfilePage({ user, logout, onNavigate }: ProfilePageProps) {
  ...
  return <div className="page profile-page">
    <section className="profile-card card">
      <div className="profile-level-avatar"><LevelAvatar className="profile-level-avatar-image" level={level} eager/><span>Lv.{level}</span></div>
      ...
      <button type="button" className="profile-growth-collection" onClick={() => setAvatarsOpen(true)}><StepsIcon weight="duotone"/><span>화랑이 성장 도감</span><small>{Math.min(level, 40)} / 40 해금</small></button>
      <div className="badges"><span>◎ 화랑이 Lv.{level}</span><span>🔥 {progress?.currentStreak ?? 0}일 연속</span></div>
